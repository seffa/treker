...
# Функция загрузки настроек (нужна для синхронизации, не меняет визуал напрямую здесь)
def load_settings(user_id):
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("CREATE TABLE IF NOT EXISTS user_settings (user_id INTEGER PRIMARY KEY, week_start TEXT DEFAULT 'Понедельник', font_size TEXT DEFAULT 'Средний')")
            ...
            if st.form_submit_button("Зарегистрироваться", use_container_width=True, type="primary"):
                if not all([nick.strip(), contact_beauty.strip(), p1.strip(), p2.strip()]):
                    ...
                    try:
                        ...
                        conn.cursor().execute("INSERT INTO user (login, password) VALUES (?, ?)", (nick.strip(), hashed))
                        new_id = conn.cursor().lastrowid
                        conn.commit()
                        st.session_state.user = {"id": new_id, "nick": nick.strip(), "contact": contact_beauty.strip()}
                        st.rerun()
                    except sqlite3.IntegrityError: st.error("Этот логин уже занят")
                    finally: conn.close()


# ---------------- 4. САЙДБАР ----------------
with st.sidebar:
    st.markdown('<div class="nav-tile"><i class="material-icons" style="font-size:40px;">menu</i></div>', unsafe_allow_html=True)
...
if st.session_state.user:
    ...
    # Подгружаем настройки в фоне (если нужно будет расширить функционал)
    _ = load_settings(u['id'])

    conn = get_db_connection()
    c = conn.cursor()
    h_count = c.execute("SELECT COUNT(*) FROM habits WHERE user_id=?", (u['id'],)).fetchone()[0]
    l_count = c.execute("SELECT COUNT(*) FROM habit_logs l JOIN habits h ON l.habit_id=h.id WHERE h.user_id=?", (u['id'],)).fetchone()[0]
    conn.close()

    achievements = [
        {"icon": "luggage", "name": "Бипки", "desc": "Что такое бипки?<br>Зарегистрироваться", "req": 0, "cur": h_count},
        {"icon": "wind_power", "name": "Воздух", "desc": "Что-то подуло<br>Вы создали первую привычку", "req": 1, "cur": l_count},
        {"icon": "lyrics", "name": "Оксимирон", "desc": "Ты выполнила задание на 5+<br>Вы создали пять привычек", "req": 5, "cur": l_count},
        {"icon": "family_restroom", "name": "Форсаж", "desc": "Последний заезд<br>Вы создали десять привычек", "req": 10, "cur": h_count},
        {"icon": "self_improvement", "name": "Смерть в нищете", "desc": "принять/принять<br>Задонатить создателям", "req": 50, "cur": l_count},
    ...
    with col_left:
        st.markdown(f'<div class="profile-container"><div class="profile-avatar">{u["nick"][0].upper()}</div><div class="user-name">{u["nick"]}</div><div class="user-mail">{u["contact"]}</div></div>', unsafe_allow_html=True)
        st.write("")
    ...
    with c2:
        st.markdown("<h3 style='text-align: center; color: #334455; margin-bottom: 20px;'>Войдите, чтобы увидеть профиль</h3>", unsafe_allow_html=True)
